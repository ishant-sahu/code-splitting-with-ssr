...
exports.default = ChunkFileNamesPlugin;
...
function ChunkFileNamesPlugin(configuration, options) {
...
ChunkFileNamesPlugin.prototype.apply = function (compiler) {
	...
	var onDone = function onDone(stats) {
		return writeChunkFileNames(stats, _this.options, _this.configuration);
	};
...
function filename_info(json, assets_base_url) {
	...
	var assets_chunks = {
		...
		.filter(function (name) {
			return _path2.default.extname(name).split('?')[0] === '.' + extension;
		})
...
function writeChunkFileNames(stats, options, webpack_configuration) {
	...
	_fsExtra2.default.outputFileSync(output_file_path, (0, _stringify2.default)(filename_info(json, assets_base_url)));
}
